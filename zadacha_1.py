...
class Presenter(ABC):
    @abstractmethod
    def show_contact(self, name, phones):
        pass
...
class Console_Presenter(Presenter):
    def show_contact(self, name, phones):
        print(f"Name: {name}\nPhones: {', '.join(phones)}\n")

    def show_contacts(self, contacts):
        for contact in contacts:
            self.show_contact(contact.name, contact.phones)
...
def main(ui_presenter):
    book = load_data()
    ui_presenter.show_message("Welcome to the assistant bot!")
    while True:
        user_input = input("Enter a command: ")
        command, *args = parse_input(user_input)

        if command in ["close", "exit"]:
            ui_presenter.show_message("Good bye!")
            save_data(book)
            break

        elif command == "hello":
            ui_presenter.show_message("How can I help you?")

        elif command == "add":
            try:
                ui_presenter.show_message(add_contact(args, book))
            except InputError as e:
                ui_presenter.show_message(e)     

        elif command == "change":
            try:
                ui_presenter.show_message(change_contact(args, book))
            except InputError as e:
                ui_presenter.show_message(e)    

        elif command == "phone":
            ui_presenter.show_message(book.show_phone(*args))  

        elif command == "all":
            ui_presenter.show_message(show_all(book))

        elif command == "add-birthday":
            try:
                ui_presenter.show_message(add_birthday(args, book))
            except InputError as e:
                ui_presenter.show_message(e)    

        elif command == "show-birthday":
            ui_presenter.show_message(show_birthday(*args, book))

        elif command == "birthdays":
            ui_presenter.show_message(birthdays(book))

        else:
            ui_presenter.show_message("Invalid command.")

if __name__ == "__main__":
    ui_presenter = Console_Presenter()
    main(ui_presenter)
